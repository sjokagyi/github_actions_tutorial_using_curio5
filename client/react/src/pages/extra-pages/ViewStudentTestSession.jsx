...
const ViewStudentTestSession = () => {
    ...
    const fetchTestSessions = async () => {
        try {
            const response = await axios.get(`http://127.0.0.1:8000/api/questions/testsessions/`, {
                headers: {
                    'Authorization': `Bearer ${user.authTokens.access}`
                },
                params: {
                    test_id: testId // Pass test_id as a query parameter
                }
            });
            console.log('Test sessions response:', response.data);
            setTestSessions(Array.isArray(response.data) ? response.data : []);
        } catch (error) {
...
export default ViewStudentTestSession;
